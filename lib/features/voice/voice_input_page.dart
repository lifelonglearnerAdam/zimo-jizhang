...
class _VoiceInputPageState extends ConsumerState<VoiceInputPage> {
  ...
  Widget _buildCatRow(int catId) {
    ...
    return _parsedRow(
      '分类',
      '${cat?.category.icon ?? ''} ${cat?.category.name ?? '未分类'}',
    );
  }
  ...
  void _parseVoiceText(String text) {
    ...
    setState(() {
      _parsedAmount = amountMatch != null
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '记账成功！${_parsedDesc ?? ""} ¥${_parsedAmount!.toStringAsFixed(2)}',
            ),
