...
class _ImportPreviewPageState extends ConsumerState<ImportPreviewPage> {
      ...
      body: Column(children: [
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            if (!_showAll)
              Text('已隐藏${state.previewEntries.where((e) => e.isDuplicate).length}条重复',
  ...
  Widget _buildStatsBar(ImportState state, bool isDark) {
    return Container(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
        ...
        _statItem('新记录', '${state.previewEntries.where((e) => !e.isDuplicate).length}条', const Color(0xFF52C41A)),
        _statItem('重复', '${state.previewEntries.where((e) => e.isDuplicate).length}条', const Color(0xFFFAAD14)),
        _statItem('已分类', '${_categoryAssignments.length}条', AppColors.textSecondary),
      ]),
    );
  }
  ...
  Widget _buildEntryRow(ImportPreviewEntry entry, bool isDark) {
    ...
    final catName = hasCategory
        ? (ref.read(allCategoriesProvider).valueOrNull ?? []).where((c) => c.id == catId).firstOrNull?.name ?? '已选'
        : null;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: entry.isDuplicate
            ...
            : Border.all(color: isDark ? const Color(0xFF334155) : const Color(0xFFE8E8ED)),
      ),
      child: Row(children: [
        // 收支图标
        Container(
          width: 36, height: 36,
          decoration: BoxDecoration(
            color: entry.type == 'income' ? AppColors.incomeLight : AppColors.expenseLight,
            borderRadius: BorderRadius.circular(9),
          ),
          alignment: Alignment.center,
          child: Icon(
            entry.type == 'income' ? Icons.arrow_downward_rounded : Icons.arrow_upward_rounded,
            size: 16,
            color: entry.type == 'income' ? AppColors.income : AppColors.danger,
          ),
        ),
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            if (entry.counterparty.isNotEmpty) ...[
              Expanded(child: Text(entry.counterparty, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
            ] else
              Expanded(child: Text(entry.description.isNotEmpty ? entry.description : '无描述', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
          ]),
          const SizedBox(height: 2),
          Row(children: [
            Text(entry.date, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
            if (entry.paymentMethod.isNotEmpty) ...[
              const SizedBox(width: 6),
              Container(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1), decoration: BoxDecoration(color: AppColors.primaryLightest, borderRadius: BorderRadius.circular(3)), child: Text(entry.paymentMethod, style: const TextStyle(fontSize: 10, color: AppColors.primary))),
            ],
            if (entry.isDuplicate) ...[
              const SizedBox(width: 6),
              Container(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1), decoration: BoxDecoration(color: Colors.red.withOpacity(0.1), borderRadius: BorderRadius.circular(3)), child: const Text('重复', style: TextStyle(fontSize: 10, color: Colors.red))),
            ],
          ]),
        ])),
        // 金额
        Text(
          ...
          style: TextStyle(
            ...
            color: entry.type == 'income' ? AppColors.income : AppColors.textPrimary,
          ),
        ),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: hasCategory ? AppColors.primaryLightest : (isDark ? const Color(0xFF334155) : const Color(0xFFF0F0F5)),
              borderRadius: BorderRadius.circular(8),
            ),
            child: Text(
              catName ?? '选分类',
              style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: hasCategory ? AppColors.primary : AppColors.textSecondary),
            ),
          ),
        ),
      ]),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) {
        ...
        final filteredParents = parentCats.where((c) {
          ...
          final isIncomeCat = c.name.contains('收入') || c.name.contains('工资') || c.name.contains('奖金');
        ...
        return Container(
          ...
          child: Column(children: [
            const Text('选择分类', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
            const SizedBox(height: 12),
            Expanded(child: ListView.builder(
              itemCount: filteredParents.length,
              itemBuilder: (_, i) {
                final parent = filteredParents[i];
                final subs = ref.read(subCategoriesProvider(parent.id)).valueOrNull ?? [];
                return ExpansionTile(
                  leading: Text(parent.icon ?? '📌', style: const TextStyle(fontSize: 20)),
                  title: Text(entry.type == 'income' ? parent.name : parent.name),
                  children: subs.map((sub) => ListTile(
                    leading: Text(sub.icon ?? '📌', style: const TextStyle(fontSize: 18)),
                    title: Text(sub.name),
                    onTap: () {
                      setState(() => _categoryAssignments[entry.index] = sub.id);
                      Navigator.pop(ctx);
                    },
                  )).toList(),
                );
  ...
  void _autoCategorize() {
    ...
    for (final entry in state.previewEntries) {
      ...
      final rules = <String, List<String>>{
        '餐饮': ['饭', '餐', '外卖', '奶茶', '咖啡', '面', '米粉', '小吃', '烧烤', '火锅', '面包', '蛋糕', '水果'],
  ...
  Widget _buildBottomBar(ImportState state) {
    final unassigned = state.previewEntries
        .where((e) => !e.isDuplicate && !_categoryAssignments.containsKey(e.index))
        .length;
    ...
    return Container(
      ...
      child: SafeArea(
        child: Row(children: [
          if (unassigned > 0)
            Text('$unassigned 条未分类', style: const TextStyle(fontSize: 12, color: AppColors.warning)),
          const Spacer(),
          TextButton(
            onPressed: () => ref.read(importProvider.notifier).reset(),
            child: const Text('取消'),
          ...
          const SizedBox(width: 8),
          ElevatedButton(
            onPressed: state.isImporting ? null : () => ref.read(importProvider.notifier).confirmImport(categoryAssignments: _categoryAssignments),
