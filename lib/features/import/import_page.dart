...
class ImportPage extends ConsumerWidget {
        ...
        child: Column(
          ...
          children: [
            ...
            _SourceCard(
              ...
              onTap: state.isParsing ? null : () => _pickAndParse(ref, 'wechat'),
            ),
            _SourceCard(
              ...
              onTap: state.isParsing ? null : () => _pickAndParse(ref, 'alipay'),
            ),
            _SourceCard(
              ...
              onTap: state.isParsing ? null : () => _pickAndParse(ref, 'bank_csv'),
            ),
            if (state.isParsing) ...[
              const SizedBox(height: 24),
              const Center(child: CircularProgressIndicator()),
              const SizedBox(height: 8),
              const Center(child: Text('正在解析账单…', style: TextStyle(color: AppColors.textSecondary))),
            ],
            if (state.error != null) ...[
  ...
  Future<void> _pickAndParse(WidgetRef ref, String source) async {
    ...
    if (result != null && result.files.isNotEmpty) {
      ...
      if (file.bytes != null) {
        await ref.read(importProvider.notifier).parseFile(file.bytes!, file.name, source);
      }
    }
  }
    ...
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          decoration: BoxDecoration(color: AppColors.primaryLightest, borderRadius: BorderRadius.circular(3)),
          child: Text(title, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.primary)),
        ),
        const SizedBox(width: 6),
        Expanded(child: Text(steps, style: const TextStyle(fontSize: 12, color: AppColors.textSecondary))),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
        side: BorderSide(color: isDark ? const Color(0xFF334155) : const Color(0xFFE8E8ED)),
      ),
      child: InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(12),
