...
class _RecurringPageState extends ConsumerState<RecurringPage> {
    ...
    return Scaffold(
      ...
      body: recurringList.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (ctx, i) =>
                  _buildRecurringCard(recurringList[i], isDark),
            ),
    ...
    return Card(
      ...
      child: ListTile(
        leading: Container(
          ...
          child: Text(
            cat?.icon ?? (rt.type == 'income' ? '💰' : '💸'),
            style: const TextStyle(fontSize: 20),
          ),
        ),
        title: Text(
          rt.description ?? (cat?.name ?? '未分类'),
        ...
        trailing: IconButton(
          ...
          onPressed: () =>
              ref.read(recurringTransactionDaoProvider).delete(rt.id!),
        ),
      ),
    );
  }
  ...
  void _showEditDialog({RecurringTransaction? existing}) {
    final amountCtrl = TextEditingController(
      text: existing != null
          ? (existing.amountFen / 100).toStringAsFixed(2)
          : '',
    );
...
/// 定期交易 Provider
final _recurringProvider = FutureProvider<List<RecurringTransaction>>((
  ref,
) async {
